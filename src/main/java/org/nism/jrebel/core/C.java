...
public interface C {
    ...
    String HTML_JS = "<script>function fn(e){var i=document.createElement('input');document.body.appendChild(i);i.setAttribute('value',e.innerText);i.select();if(document.execCommand('copy')){alert(e.innerText+'\\n复制文本到剪切板')}document.body.removeChild(i);location.reload()}</script>";
